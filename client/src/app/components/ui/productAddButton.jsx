...
import { createOrder, getOrderById, updateOrder } from '../../store/orders';
...
const ProductAddButton = ({ product, full }) => {
	...
	const orderProduct = useSelector(getOrderById(product._id)) ?? null;
	const handleClick = () => {
		if (isLoggedIn) {
	...
	const handleIncrement = () => {
		if (orderProduct[1] + 1 <= product.quantity) {
	...
	const handleDecrement = () => {
		if (orderProduct[1] - 1 > 0) {
			...
			dispatch(updateOrder(product._id, 'delete'));
		}
	};
